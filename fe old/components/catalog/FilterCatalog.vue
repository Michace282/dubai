    ...
    export default {
        ...
        created() {
            ...
            if (this.$route.params.product) {
                ...
                var productTypeNew = this.$route.params.product;
                if (this.$route.params.product == 'womens_dancewear')
                 productTypeNew = 'ladies'
                if (this.$route.params.product == 'mens_dancewear')
                 productTypeNew = 'mens'
                if (this.$route.params.product == 'accessories')
                 productTypeNew = 'accessories'
                if (this.$route.params.product == 'dance_shoes')
                 productTypeNew = 'dance_shoes' 


                this.filter.productType = productTypeNew;
                ...
                for (let key in this.filter) {
                    ...
                    if (Array.isArray(this.filter[key])) {
                        if (this.filter[key].length > 0) {
                            ...
                            if (['colors', 'sizes', 'price_Gte','ladiesType','mensType', 'accessoriesType','danceShoesType','price_Lte', 'isNew'].indexOf(key) != -1) {
                                queryFilter[key] = this.filter[key];
                            }
                        }
                    } else if (this.filter[key]) {
                        activeFilter[key] = this.filter[key];
                        if (['colors', 'sizes', 'price_Gte', 'price_Lte', 'isNew','ladiesType','mensType', 'accessoriesType','danceShoesType'].indexOf(key) != -1) {
                            queryFilter[key] = this.filter[key];
                        }
                    }
